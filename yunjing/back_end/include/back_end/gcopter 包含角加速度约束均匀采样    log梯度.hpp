...
rightedgePub = nh.advertise<visualization_msgs::Marker>("/visualizer/rightedge", 1000);
  ...
  inline void get_hPoly(){
    ...
    for(int i=0; i<traj_num; i++){
      ...
      piece_nums = std::max(int(initTotalduration / config_.timeResolution_ + 0.5),2);
      // 再均匀分小段
      ...
      for (int j = 0; j <= K; ++j){
        ...
        if(violaOmegaL > 0.0){
          ...
          gradViolaKLc = 1 / (violaOmegaL + 1) * (
                                  beta1 * (vel2_reci_e * ddsigma.transpose()*B_h - 2 * vel2_reci_e * vel2_reci_e * z_h3 * dsigma.transpose() + dsigma.transpose() * sqrt(vel2_reci_e)/config_.wheel_base_) 
                                  + beta2 * vel2_reci_e * dsigma.transpose() * B_h.transpose()); // 6*2
          gradViolaKLt  = alpha/ (violaOmegaL + 1)*( vel2_reci_e*(dddsigma.transpose()*B_h*dsigma - 2*vel2_reci_e*z_h3*z_h1) + dsigma.transpose() * sqrt(vel2_reci_e)/config_.wheel_base_ * ddsigma);
        ...
        if(violaOmegaR > 0.0){
          ...
          gradViolaKRc = 1 / (violaOmegaR + 1) * (
                                  -(beta1 * (vel2_reci_e * ddsigma.transpose()*B_h - 2 * vel2_reci_e * vel2_reci_e * z_h3 * dsigma.transpose() - dsigma.transpose() * sqrt(vel2_reci_e)/config_.wheel_base_) 
                                  + beta2 * vel2_reci_e * dsigma.transpose() * B_h.transpose())); // 6*2
          gradViolaKRt  = -alpha/ (violaOmegaL + 1)*( vel2_reci_e*(dddsigma.transpose()*B_h*dsigma - 2*vel2_reci_e*z_h3*z_h1) - dsigma.transpose() * sqrt(vel2_reci_e)/config_.wheel_base_ * ddsigma );
