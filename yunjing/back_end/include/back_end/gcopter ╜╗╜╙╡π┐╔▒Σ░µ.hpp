...
rightedgePub = nh.advertise<visualization_msgs::Marker>("/visualizer/rightedge", 1000);
  ...
  inline void get_hPoly(){
    ...
    for(int i=0; i<traj_num; i++){
      ...
      piece_nums = std::max(int(initTotalduration / config_.timeResolution_ + 0.5),4);
      // 再均匀分小段
  ...
  inline void optimizer(){
    ...
    variable_num_ += 3 * (piece_singul_num_-1);// 焦点x y坐标和转角theta
    ...
    RealT2VirtualT(pieceTimes,Vt);
    offset += pieceTimes.size();
    // 变转点
    for(int i=1; i<piece_singul_num_; i++){
      Eigen::Map<Eigen::Vector3d> xytheta(x.data()+offset, 3);
      xytheta<<iniStates[i](0,0),iniStates[i](1,0),atan2(singuls[i]*iniStates[i](1,1),singuls[i]*iniStates[i](0,1));
    }
    ...
    int result = lbfgs::lbfgs_optimize(x,
                                       ...
                                       this,
                                       lbfgs_params);
    ROS_INFO("optimizer finish! result:%d   finalcost:%f",result,cost);
    ...
    for(int i=0; i<piece_singul_num_; i++){
      ...
      double t = x[2*(eachTrajNums.sum()-piece_singul_num_)+i];
      ...
      if(piece_singul_num_!=1){
        if(i != piece_singul_num_-1){
          Eigen::Map<Eigen::Vector3d> turningP(x.data() + 2*(eachTrajNums.sum()-piece_singul_num_)+piece_singul_num_+3*i, 3);
          Eigen::MatrixXd tailState(2,3);
          tailState.setZero();
          tailState.col(0) = turningP.head(2);
          tailState.col(1) = config_.non_siguav_ * singuls[i] * Eigen::Vector2d(cos(turningP.z()),sin(turningP.z()));
          mincos[i].setTConditions(tailState);
        }
        if(i != 0){
          Eigen::Map<Eigen::Vector3d> turningP(x.data() + 2*(eachTrajNums.sum()-piece_singul_num_)+piece_singul_num_+3*(i-1), 3);
          Eigen::MatrixXd headState(2,3);
          headState.setZero();
          headState.col(0) = turningP.head(2);
          headState.col(1) = config_.non_siguav_ * singuls[i] * Eigen::Vector2d(cos(turningP.z()),sin(turningP.z()));
          mincos[i].setHConditions(headState);
        }
      }
  ...
  inline void VirtualT2RealT(const EIGENVEC &VT, Eigen::VectorXd &RT)
  {
    ...
    // std::cout<<"VT: "<<VT.transpose()<<"   RT: "<<RT.transpose()<<std::endl;
  }
  ...
  static double costFunctionCallback(void *ptr,
    ...
    std::vector<Eigen::Map<const Eigen::Vector3d>> turningP_container;
    std::vector<Eigen::Map<Eigen::Vector3d>> gradturningP_container;
    ...
    for(int trajid = 1; trajid < obj.piece_singul_num_; trajid++){
      Eigen::Map<const Eigen::Vector3d> turningP(x.data() + offset, 3);
      Eigen::Map<Eigen::Vector3d> gradturningP(g.data() + offset, 3);
      offset+=3;
      gradturningP.setZero();
      turningP_container.push_back(turningP);
      gradturningP_container.push_back(gradturningP);
    }
    ...
    for(int trajid = 0; trajid < obj.piece_singul_num_; trajid++){
      ...
      double cost;
      if(obj.piece_singul_num_!=1){
        if(trajid != obj.piece_singul_num_-1){
          Eigen::MatrixXd tailState(2,3);
          tailState.setZero();
          tailState.col(0) = turningP_container[trajid].head(2);
          tailState.col(1) = obj.config_.non_siguav_ * obj.singuls[trajid] * Eigen::Vector2d(cos(turningP_container[trajid].z()),sin(turningP_container[trajid].z()));
          obj.mincos[trajid].setTConditions(tailState);
        }
        if(trajid != 0){
          Eigen::MatrixXd headState(2,3);
          headState.setZero();
          headState.col(0) = turningP_container[trajid-1].head(2);
          headState.col(1) = obj.config_.non_siguav_ * obj.singuls[trajid] * Eigen::Vector2d(cos(turningP_container[trajid-1].z()),sin(turningP_container[trajid-1].z()));
          obj.mincos[trajid].setHConditions(headState);
        }
      }
      ...
      if(obj.piece_singul_num_!=1){
        if(trajid != obj.piece_singul_num_-1){
          Eigen::Vector2d gradtheta(-obj.config_.non_siguav_ * sin(turningP_container[trajid][2]),obj.config_.non_siguav_ * cos(turningP_container[trajid][2]));
          gradturningP_container[trajid][2] += obj.singuls[trajid]*gradtheta.transpose()*gradTail.col(1);

          gradturningP_container[trajid].head(2) += gradTail.col(0);
        }
        if(trajid != 0){
          Eigen::Vector2d gradtheta(-obj.config_.non_siguav_ * sin(turningP_container[trajid-1][2]),obj.config_.non_siguav_ * cos(turningP_container[trajid-1][2]));
          gradturningP_container[trajid-1][2] += obj.singuls[trajid]*gradtheta.transpose()*gradInit.col(1);

          gradturningP_container[trajid-1].head(2) += gradInit.col(0);
        }
      }
    }
  ...
  void attachPenaltyFunctional(const int &trajid, double &cost){
    ...
    double cost_corr=0, cost_v=0, cost_a=0, cost_cur=0;
    for (int i = 0; i < N; ++i){
      int K;
      if(i==0 || i==N-1)
      ...
      for (int j = 0; j <= K; ++j){
        ...
        // zmk
        double omega = z_h3 * vel3_2_reci_e;
        double max_omega_ = 2*(config_.max_vel_- std::abs(z_h0))/config_.wheel_base_;
        ...
        for (auto cp : leftMani_check_point){        
          ...
          for (int k = 0; k < corr_k; k++){
            ...
            violaPos = (outerNormal.dot(bpt) + leftcfgHs[pointid].col(k)[2]*cp.z() + leftcfgHs[pointid].col(k)[3])+ safeDis;
        ...
        for (auto cp : rightMani_check_point){     
          ...
          for (int k = 0; k < corr_k; k++){
            ...
            violaPos = (outerNormal.dot(bpt) + rightcfgHs[pointid].col(k)[2]*cp.z() + rightcfgHs[pointid].col(k)[3])+ safeDis;
            if (violaPos > 0.0){
        ...
        if(violaOmegaL > 0.0){
          positiveSmoothedL1(violaOmegaL, violaOmegaL, violaOmegaPenaDL);
          //zmk
          gradViolaKLc = beta1 * (vel2_reci_e * ddsigma.transpose()*B_h - 3 * vel2_reci_e * vel2_reci_e * z_h3 * dsigma.transpose()) 
                         + beta2 * vel2_reci_e * dsigma.transpose() * B_h.transpose(); // 6*2
          gradViolaKLt  = alpha*vel2_reci_e*(dddsigma.transpose()*B_h*dsigma - 2*vel2_reci_e*z_h3*z_h1);
          partialGradByCoeffs.block<6,2>(i*6 , 0) += omg * step * penaltyWt[3] * violaOmegaPenaDL * gradViolaKLc;
          partialGradByTimes(i) += omg * penaltyWt[3] * (violaOmegaPenaDL * gradViolaKLt * step + violaOmegaL / K);
          cost += omg * step * penaltyWt[3] * violaOmegaL;
          // curcost+=omg * step * wei_feas_ * violaOmegaL;
        }
        if(violaOmegaR > 0.0){
          positiveSmoothedL1(violaOmegaR, violaOmegaR, violaOmegaPenaDR);
          //zmk
          gradViolaKRc = -(beta1 * (vel2_reci_e * ddsigma.transpose()*B_h - 3 * vel2_reci_e * vel2_reci_e * z_h3 * dsigma.transpose()) 
                         + beta2 * vel2_reci_e * dsigma.transpose() * B_h.transpose()); // 6*2
          gradViolaKRt  = -(alpha*vel2_reci_e*(dddsigma.transpose()*B_h*dsigma-2*vel2_reci_e*z_h3*z_h1));
          partialGradByCoeffs.block<6,2>(i*6 , 0) += omg * step * penaltyWt[3] * violaOmegaPenaDR * gradViolaKRc;
          partialGradByTimes(i) += omg * penaltyWt[3] * (violaOmegaPenaDR * gradViolaKRt * step + violaOmegaR / K);
          cost += omg * step * penaltyWt[3] * violaOmegaR;
          // curcost+=omg * step * wei_feas_ * violaOmegaR;
        }
      }
