...
rightedgePub = nh.advertise<visualization_msgs::Marker>("/visualizer/rightedge", 1000);
    ...
    leftMani_check_point.emplace_back(0.025 + config_.length_/2-config_.rear_suspension_,0.1,0.2);
    leftMani_check_point.emplace_back(0.025 + config_.length_/2-config_.rear_suspension_,0.15,0.2);
    leftMani_check_point.emplace_back(-0.025 + config_.length_/2-config_.rear_suspension_,0.15,0.2);
    leftMani_check_point.emplace_back(-0.025 + config_.length_/2-config_.rear_suspension_,0.1,0.2);
    ...
    rightMani_check_point.emplace_back(0.025 + config_.length_/2-config_.rear_suspension_,-0.1,0.2);
    rightMani_check_point.emplace_back(0.025 + config_.length_/2-config_.rear_suspension_,-0.15,0.2);
    rightMani_check_point.emplace_back(-0.025 + config_.length_/2-config_.rear_suspension_,-0.15,0.2);
    rightMani_check_point.emplace_back(-0.025 + config_.length_/2-config_.rear_suspension_,-0.1,0.2);
  ...
  inline void minco_plan(){
    ...
    get_hPoly();
    ROS_INFO("\033[41;37m get_hPoly time:%f \033[0m", (ros::Time::now()-current).toSec());
    vis_2Dbox();
    // vis_3Dbox();
    vis_right3Dbox();
    vis_left3Dbox();
  ...
  inline void get_hPoly(){
    ...
    for(int i=0; i<traj_num; i++){
      ...
      // ROS_INFO("\033[42;38m get 2Drectangle time:%f \033[0m", (ros::Time::now()-current).toSec());
      ...
      // ROS_INFO("\033[42;38m get ManiCorridor time:%f \033[0m", (ros::Time::now()-current).toSec());
  ...
  inline void optimizer(){
    ...
    RealT2VirtualT(pieceTimes*1.5,Vt);
...
// std::cout<<"g: "<<g.transpose()<<std::endl;
  ...
  void attachPenaltyFunctional(const int &trajid, double &cost){
    ...
    // std::cout<<"----------------------------attachPenaltyFunctional----------------------------"<<std::endl;
    ...
    for (int i = 0; i < N; ++i){
      ...
      for (int j = 0; j <= K; ++j){
        ...
        double max_omega_ = 2*(config_.max_vel_- dsigma.norm())/config_.wheel_base_/20.0;
        ...
        if(violaOmegaL > 0.0){
          double logviolaOmegaL = log(violaOmegaL+1);
          positiveSmoothedL1(logviolaOmegaL, violaOmegaPenaL, violaOmegaPenaDL);
          //zmk
          gradViolaKLc = 1 / (violaOmegaL + 1) * (
                                  beta1 * (vel2_reci_e * ddsigma.transpose()*B_h - 2 * vel2_reci_e * vel2_reci_e * z_h3 * dsigma.transpose() + 2.0/20.0 * dsigma.transpose() * sqrt(vel2_reci_e)/config_.wheel_base_) 
                                  + beta2 * vel2_reci_e * dsigma.transpose() * B_h.transpose()); // 6*2
          gradViolaKLt  = alpha/ (violaOmegaL + 1)*( vel2_reci_e*(dddsigma.transpose()*B_h*dsigma - 2*vel2_reci_e*z_h3*z_h1) + 2/20.0 * dsigma.transpose() * sqrt(vel2_reci_e)/config_.wheel_base_ * ddsigma);
        ...
        if(violaOmegaR > 0.0){
          double logviolaOmegaR = log(violaOmegaR+1);
          positiveSmoothedL1(logviolaOmegaR, violaOmegaPenaR, violaOmegaPenaDR);
          //zmk
          gradViolaKRc = 1 / (violaOmegaR + 1) * (
                                  -(beta1 * (vel2_reci_e * ddsigma.transpose()*B_h - 2 * vel2_reci_e * vel2_reci_e * z_h3 * dsigma.transpose() - 2.0/20.0 * dsigma.transpose() * sqrt(vel2_reci_e)/config_.wheel_base_) 
                                  + beta2 * vel2_reci_e * dsigma.transpose() * B_h.transpose())); // 6*2
          gradViolaKRt  = -alpha/ (violaOmegaL + 1)*( vel2_reci_e*(dddsigma.transpose()*B_h*dsigma - 2*vel2_reci_e*z_h3*z_h1) - 2/20.0 * dsigma.transpose() * sqrt(vel2_reci_e)/config_.wheel_base_ * ddsigma );
        ...
        if( (i!=0||j!=0) && (i!=N-1||j!=K)){
          omg = (i==0&&j==1 || i==N-1&&j==K-1 || j == 0 || j == K) ? 0.5 : 1.0;
          if(violadOmegaL > 0.0){
            // std::cout<<"violadOmegaL"<<violadOmegaL<<std::endl;
            double logvioladOmegaL = log(violadOmegaL+1);
            positiveSmoothedL1(logvioladOmegaL, violadOmegaPenaL, violadOmegaPenaDL);
            //zmk
            gradViolaKLc = 1 / (violadOmegaL+1) * (
                          beta3 * (dsigma.transpose()*B_h.transpose() * vel2_reci_e) +
                          ...
                          beta1 * ((dddsigma.transpose() * B_h * vel2_reci_e) 
                                    - vel2_reci_e * vel2_reci_e * 2.0 * ( z_h41 * dsigma.transpose() +
                                                                          z_h1 * ddsigma.transpose()*B_h + z_h3 * ddsigma.transpose())
                                    + 8.0 * z_h3 * z_h1 * dsigma.transpose() * vel2_reci_e * vel2_reci_e * vel2_reci_e));
            gradViolaKLt = alpha / (violadOmegaL+1) * ((z_h5 + z_h6) * vel2_reci_e + 
                                                      -(ddsigma.squaredNorm()*z_h3 + z_h2*z_h3 + z_h41*2.0*z_h1) * 2.0  * vel2_reci_e * vel2_reci_e + 
          ...
          if(violadOmegaR > 0.0){
            // std::cout<<"violadOmegaR"<<violadOmegaR<<std::endl;
            double logvioladOmegaR = log(violadOmegaR+1);
            positiveSmoothedL1(logvioladOmegaR, violadOmegaPenaR, violadOmegaPenaDR);
            //zmk
            gradViolaKRc = 1 / (violadOmegaR+1) * (
                          -beta3 * (dsigma.transpose()*B_h.transpose() * vel2_reci_e) +
                          beta2 * 2.0 * vel2_reci_e * vel2_reci_e * ( z_h1 * dsigma.transpose()*B_h.transpose() + z_h3 * dsigma.transpose()) +
                          -beta1 * ((dddsigma.transpose() * B_h * vel2_reci_e) 
                                    - vel2_reci_e * vel2_reci_e * 2.0 * ( z_h41 * dsigma.transpose() +
                                                                          z_h1 * ddsigma.transpose()*B_h + z_h3 * ddsigma.transpose())
                                    + 8.0 * z_h3 * z_h1 * dsigma.transpose() * vel2_reci_e * vel2_reci_e * vel2_reci_e));
            gradViolaKRt = -alpha / (violadOmegaR+1) * ((z_h5 + z_h6) * vel2_reci_e + 
                                                        -(ddsigma.squaredNorm()*z_h3 + z_h2*z_h3 + z_h41*2.0*z_h1) * 2.0  * vel2_reci_e * vel2_reci_e + 
                                                        8.0 * z_h3 * z_h1 * z_h1 * vel2_reci_e * vel2_reci_e * vel2_reci_e);
        ...
        if( (i==0&&j==0) || (i==N-1&&j==K-1)){
          int special_resolution_ = 10;
          double special_step = step / special_resolution_;
          
          double special_s1 = s1 - step;
          for(int l=0; l<=special_resolution_; l++){
            s2 = special_s1 * special_s1;
            s3 = s2 * special_s1;
            s4 = s2 * s2;
            s5 = s4 * special_s1;
            beta0 << 1.0, special_s1, s2, s3, s4, s5;
            beta1 << 0.0, 1.0, 2.0 * special_s1, 3.0 * s2, 4.0 * s3, 5.0 * s4;
            beta2 << 0.0, 0.0, 2.0, 6.0 * special_s1, 12.0 * s2, 20.0 * s3;
            beta3 << 0.0, 0.0, 0.0, 6.0, 24.0 * special_s1, 60.0 * s2;
            beta4 << 0.0, 0.0, 0.0, 0, 24.0, 120.0 * special_s1;
            alpha = 1.0 / K * j + 1.0 / K / special_resolution_ * j * l;
            // std::cout<<"special_step:"<<special_step<<"      step:"<<step<<"        special_s1:"<<special_s1<<std::endl;
            ...
            omg = (l == 0 || l == special_resolution_) ? 0.5 : 1.0;
            z_h0 = dsigma.norm();
            z_h1 = ddsigma.transpose() * dsigma;
            z_h2 = dddsigma.transpose() * dsigma;
            z_h3 = ddsigma.transpose() * B_h * dsigma;
            ...
            domega = z_h41*vel2_reci_e - 2.0 * z_h3 * z_h1 * vel2_reci_e * vel2_reci_e;
            violadOmegaL = domega - config_.max_domega_;
            violadOmegaR = -domega - config_.max_domega_;
            if(violadOmegaL > 0.0){
              // std::cout<<"violadOmegaL"<<violadOmegaL<<"     domega:"<<domega<<std::endl;
              double logvioladOmegaL = log(violadOmegaL+1);
              positiveSmoothedL1(logvioladOmegaL, violadOmegaPenaL, violadOmegaPenaDL);
              //zmk
              gradViolaKLc = 1 / (violadOmegaL+1) * (
              ...
              gradViolaKLt = alpha / (violadOmegaL+1) * ((z_h5 + z_h6) * vel2_reci_e + 
                                                        -(ddsigma.squaredNorm()*z_h3 + z_h2*z_h3 + z_h41*2.0*z_h1) * 2.0  * vel2_reci_e * vel2_reci_e + 
                                                        8.0 * z_h3 * z_h1 * z_h1 * vel2_reci_e * vel2_reci_e * vel2_reci_e);
              partialGradByCoeffs.block<6,2>(i*6 , 0) += omg * special_step * penaltyWt[5] * violadOmegaPenaDL * gradViolaKLc;
              partialGradByTimes(i) += omg * penaltyWt[5] * (violadOmegaPenaDL * gradViolaKLt * special_step + violadOmegaPenaL / K / special_resolution_);
              cost += omg *special_step * penaltyWt[5] * violadOmegaPenaL;
              cost_domega += omg * special_step * penaltyWt[5] * violadOmegaPenaL;
            }
            if(violadOmegaR > 0.0){
              // std::cout<<"violadOmegaR"<<violadOmegaR<<"     domega:"<<domega<<std::endl;
              double logvioladOmegaR = log(violadOmegaR+1);
              positiveSmoothedL1(logvioladOmegaR, violadOmegaPenaR, violadOmegaPenaDR);
              //zmk
              gradViolaKRc = 1 / (violadOmegaR+1) * (
              ...
              gradViolaKRt = -alpha / (violadOmegaR+1) * ((z_h5 + z_h6) * vel2_reci_e + 
                                                          -(ddsigma.squaredNorm()*z_h3 + z_h2*z_h3 + z_h41*2.0*z_h1) * 2.0  * vel2_reci_e * vel2_reci_e + 
                                                          8.0 * z_h3 * z_h1 * z_h1 * vel2_reci_e * vel2_reci_e * vel2_reci_e);
              partialGradByCoeffs.block<6,2>(i*6 , 0) += omg * special_step * penaltyWt[5] * violadOmegaPenaDR * gradViolaKRc;
              partialGradByTimes(i) += omg * penaltyWt[5] * (violadOmegaPenaDR * gradViolaKRt * special_step + violadOmegaPenaR / K / special_resolution_);
              cost += omg * special_step * penaltyWt[5] * violadOmegaPenaR;
              cost_domega +=omg * special_step * penaltyWt[5] * violadOmegaPenaR;
            }
          } 
        }
