...
public:
  ...
  inline void optimizer(){
    ...
    final_trajes.clear();
    // int offset = 0;
    for(traj_num_=0; traj_num_<piece_singul_num_; traj_num_++){
      x.resize(Innerpointses[traj_num_].size()+1);
      memcpy(x.data(),Innerpointses[traj_num_].data(), Innerpointses[traj_num_].size() * sizeof(x[0]));
      // offset += Innerpointses[traj_num_].size();
      x[x.size()-1] = Vt[traj_num_];
      ...
      Eigen::Map<const Eigen::MatrixXd> P(x.data(), 2, eachTrajNums[traj_num_] - 1);
      double t = x[x.size()-1];
      double T = t > 0
              ? (t*(t/2+1)+1)
              : (2/(t*(t-2)+2));
      mincos[traj_num_].setParameters(P,T/eachTrajNums[traj_num_]);
      mincos[traj_num_].getTrajectory(final_traj);
      final_trajes.push_back(final_traj);
      finalInnerpointses.emplace_back(P);

      std::cout<<"g:    "<<g.transpose()<<std::endl<<std::endl;
    }
  ...
  static double costFunctionCallback(void *ptr,
    ...
    int trajid = obj.traj_num_;

    g.setZero();
    // 将输入变量映射到变量矩阵
    int offset = 0;
    
    Eigen::Map<const Eigen::MatrixXd> P(x.data(), 2, obj.eachTrajNums[trajid] - 1);
    Eigen::Map<Eigen::MatrixXd> gradP(g.data(), 2, obj.eachTrajNums[trajid] - 1);
    gradP.setZero();
    
    const double t = x[x.size()-1];
    // double gradt;
    // Eigen::Map<Eigen::VectorXd> gradt(g.data()+x.size()-1, 1);
    double T;

    T = t > 0.0 ? ((0.5 * t + 1.0) * t + 1.0)
                  : 1.0 / ((0.5 * t - 1.0) * t + 1.0);
    ...
    double pieceT;
    pieceT = T/obj.eachTrajNums[trajid];
    double costOfAll = 0;
    // std::cout<<"-------------------------------------------------------------"<<std::endl;
      ...
      double cost;
      obj.mincos[trajid].setParameters(P,pieceT);
      ...
      backwardGradT(t, obj.gradOneTime, g[x.size()-1]);
      ...
      for (int j = 0; j <= K; ++j){
        ...
        for (auto cp : leftMani_check_point){        
          ...
          for (int k = 0; k < corr_k; k++){
            ...
            violaPos = (outerNormal.dot(bpt) + leftcfgHs[pointid].col(k)[2]*cp.z() + leftcfgHs[pointid].col(k)[3]);
        ...
        for (auto cp : rightMani_check_point){     
          ...
          for (int k = 0; k < corr_k; k++){
            ...
            violaPos = (outerNormal.dot(bpt) + rightcfgHs[pointid].col(k)[2]*cp.z() + rightcfgHs[pointid].col(k)[3]);
            if (violaPos > 0.0){
        ...
        if(violaOmegaL > 0.0){
          ...
          //zmk
          gradViolaKLc = beta1 * (vel2_reci_e * ddsigma.transpose()*B_h - 2 * vel2_reci_e * vel2_reci_e * z_h3 * dsigma.transpose() + dsigma.transpose() * sqrt(vel2_reci_e)) 
                         + beta2 * vel2_reci_e * dsigma.transpose() * B_h.transpose(); // 6*2
          gradViolaKLt  = alpha*( vel2_reci_e*(dddsigma.transpose()*B_h*dsigma - 2*vel2_reci_e*z_h3*z_h1) + dsigma.transpose() * sqrt(vel2_reci_e) * ddsigma);
        ...
        if(violaOmegaR > 0.0){
          ...
          //zmk
          gradViolaKRc = -(beta1 * (vel2_reci_e * ddsigma.transpose()*B_h - 2 * vel2_reci_e * vel2_reci_e * z_h3 * dsigma.transpose() + dsigma.transpose() * sqrt(vel2_reci_e)) 
                           + beta2 * vel2_reci_e * dsigma.transpose() * B_h.transpose()); // 6*2
          gradViolaKRt  = -(alpha*( vel2_reci_e*(dddsigma.transpose()*B_h*dsigma - 2*vel2_reci_e*z_h3*z_h1) + dsigma.transpose() * sqrt(vel2_reci_e) * ddsigma));
