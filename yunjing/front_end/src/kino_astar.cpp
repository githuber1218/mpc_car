...
// ROS_INFO("\033[43;37m x_upper:%f x_lower:%f y_upper:%f y_lower:%f z_upper:%f z_lower:%f \033[0m",x_upper,x_lower,y_upper,y_lower,z_upper,z_lower);
  ...
  nh_.param<double>(ros::this_node::getName()+ "/astar_cur",astar_cur_,0.5);
  ...
  max_steer_ = std::atan(wheel_base_*astar_cur_);
...
// ROS_INFO("\033[41;37m path:%s width_:%f \033[0m",(ros::this_node::getName()+ "width").c_str(),width_);  
  shotptr_ =std::make_shared<ompl::base::ReedsSheppStateSpace>(1.0/(astar_cur_/1.5));
...
// ROS_INFO("\033[43;37m runTime:%f max_search_time_:%f \033[0m",runTime,max_search_time_);
    ...
    // 初始搜索和之后的搜索采样标准不同（因为初值可能有正向的速度）  获得输入 input第一项是旋转角度，第二项是位移
    if(!initsearch ){
      if(start_state_[3]>0){
      ...
      for (double arc = -step_arc_; arc <= step_arc_ + 1e-3; arc += 0.5*step_arc_){
...
//   Eigen::Vector2d center(pos + egoR*Eigen::Vector2d(length_/2-rear_suspension_,0));
...
bool KinoAstar::IfCollision_PosYaw(const Eigen::Vector2d &pos, const double &yaw){
  ...
  for(double height = 0; height<=height_; height+=Collision_interval_){
    check_occ = map_->CheckCollisionBycoord(corner1.x(),corner1.y(),height);
    if(check_occ==1){
      return true;
    }
    check_occ = map_->CheckCollisionBycoord(corner2.x(),corner2.y(),height);
    if(check_occ==1){
      return true;
    }
    check_occ = map_->CheckCollisionBycoord(corner3.x(),corner3.y(),height);
    if(check_occ==1){
      return true;
    }
    check_occ = map_->CheckCollisionBycoord(corner4.x(),corner4.y(),height);
    if(check_occ==1){
      return true;
    }
    ...
    for(double dl = Collision_interval_;dl < norm12;dl+=Collision_interval_){
      point = dl / norm12 * (corner2-corner1) + corner1;
      check_occ = map_->CheckCollisionBycoord(point.x(),point.y(),height);
      if(check_occ==1){
        return true;
      }
    }
  ...
  // 机臂
  Eigen::Vector2d mancorner11 = center + egoR*Eigen::Vector2d(0.05,-0.1);
  Eigen::Vector2d mancorner12 = center + egoR*Eigen::Vector2d(0.05,-0.15);
  Eigen::Vector2d mancorner13 = center + egoR*Eigen::Vector2d(-0.05,-0.15);
  Eigen::Vector2d mancorner14 = center + egoR*Eigen::Vector2d(-0.05,-0.1);
  Eigen::Vector2d mancorner21 = center + egoR*Eigen::Vector2d(0.05,0.1);
  Eigen::Vector2d mancorner22 = center + egoR*Eigen::Vector2d(0.05,0.15);
  Eigen::Vector2d mancorner23 = center + egoR*Eigen::Vector2d(-0.05,0.15);
  Eigen::Vector2d mancorner24 = center + egoR*Eigen::Vector2d(-0.05,0.1);

  double norm = 0.05;

  for(double height = 0.2; height<=0.3; height+=Collision_interval_){
    check_occ = map_->CheckCollisionBycoord(mancorner11.x(),mancorner11.y(),height);
    if(check_occ==1){
      return true;
    }
    check_occ = map_->CheckCollisionBycoord(mancorner12.x(),mancorner12.y(),height);
    if(check_occ==1){
      return true;
    }
    check_occ = map_->CheckCollisionBycoord(mancorner13.x(),mancorner13.y(),height);
    if(check_occ==1){
      return true;
    }
    check_occ = map_->CheckCollisionBycoord(mancorner14.x(),mancorner14.y(),height);
    if(check_occ==1){
      return true;
    }
    ...
    for(double dl = Collision_interval_;dl < norm;dl+=Collision_interval_){
      point = dl / norm * (mancorner12-mancorner11) + mancorner11;
      check_occ = map_->CheckCollisionBycoord(point.x(),point.y(),height);
      if(check_occ==1){
        return true;
      }
    ...
    for(double dl = Collision_interval_;dl < norm;dl+=Collision_interval_){
      point = dl / norm * (mancorner13-mancorner12) + mancorner12;
      check_occ = map_->CheckCollisionBycoord(point.x(),point.y(),height);
      if(check_occ==1){
        return true;
      }
    ...
    for(double dl = Collision_interval_;dl < norm;dl+=Collision_interval_){
      point = dl / norm * (mancorner14-mancorner13) + mancorner13;
      check_occ = map_->CheckCollisionBycoord(point.x(),point.y(),height);
      if(check_occ==1){
        return true;
      }
    ...
    for(double dl = Collision_interval_;dl < norm;dl+=Collision_interval_){
      point = dl / norm * (mancorner11-mancorner14) + mancorner14;
      check_occ = map_->CheckCollisionBycoord(point.x(),point.y(),height);
      if(check_occ==1){
        return true;
      }
    ...
    check_occ = map_->CheckCollisionBycoord(mancorner21.x(),mancorner21.y(),height);
    if(check_occ==1){
      return true;
    }
    check_occ = map_->CheckCollisionBycoord(mancorner22.x(),mancorner22.y(),height);
    if(check_occ==1){
      return true;
    }
    check_occ = map_->CheckCollisionBycoord(mancorner23.x(),mancorner23.y(),height);
    if(check_occ==1){
      return true;
    }
    check_occ = map_->CheckCollisionBycoord(mancorner24.x(),mancorner24.y(),height);
    if(check_occ==1){
      return true;
    }

    for(double dl = Collision_interval_;dl < norm;dl+=Collision_interval_){
      point = dl / norm * (mancorner22-mancorner21) + mancorner21;
      check_occ = map_->CheckCollisionBycoord(point.x(),point.y(),height);
      if(check_occ==1){
        return true;
      }
    ...
    for(double dl = Collision_interval_;dl < norm;dl+=Collision_interval_){
      point = dl / norm * (mancorner23-mancorner22) + mancorner22;
      check_occ = map_->CheckCollisionBycoord(point.x(),point.y(),height);
      if(check_occ==1){
        return true;
      }
    ...
    for(double dl = Collision_interval_;dl < norm;dl+=Collision_interval_){
      point = dl / norm * (mancorner24-mancorner23) + mancorner23;
      check_occ = map_->CheckCollisionBycoord(point.x(),point.y(),height);
      if(check_occ==1){
        return true;
      }
    ...
    for(double dl = Collision_interval_;dl < norm;dl+=Collision_interval_){
      point = dl / norm * (mancorner21-mancorner24) + mancorner24;
      check_occ = map_->CheckCollisionBycoord(point.x(),point.y(),height);
      if(check_occ==1){
        return true;
      }
...
inline double KinoAstar::getHeu(const Eigen::VectorXd &x1, const Eigen::VectorXd &x2){
...
bool KinoAstar::is_shot_sucess(const Eigen::Vector3d &state1, const Eigen::Vector3d &state2){
...
void KinoAstar::getKinoNode(){
  ...
  for(int i=0;i<shot_lengthList.size();i++){
    ...
    for(samplet = sampletime_; samplet<shot_timeList[i]; samplet+=sampletime_){
      ...
      for(int k = index; k<localTraj.size()-1 ;k++)
      {
        ...
        if(tmparc>=arc){
          ...
          traj_pts.push_back(Eigen::Vector3d(px,py,sampletime_));
    ...
    traj_pts.push_back(Eigen::Vector3d(localTraj.back()[0],localTraj.back()[1],shot_timeList[i]-(samplet-sampletime_)));
  ...
  for(int i=0;i<shot_timeList.size();i++){
    ...
    // std::cout<<"shot_timeList[i]:       "<<shot_timeList[i]<<std::endl;
  }
  // std::cout<<"totalTrajTime:       "<<totalTrajTime<<std::endl;
}
...
Eigen::Vector3d KinoAstar::evaluatePos(const double &input_t){
  ...
  double tmpT = 0;
  double CutTime;
    //locate the local traj 找到当前时间所在的段
  for(int i = 0;i<shot_timeList.size();i++){
  ...
  // 从转折点坐标数组中找到两个转折点从而选出两个点
  int front = shotindex[index]; int back =  shotindex[index+1];
  // 将t所在的轨迹段择出来   初始速度为零点到末尾速度为零点
  std::vector<Eigen::Vector3d> localTraj;localTraj.assign(sample_trajs_.begin()+front,sample_trajs_.begin()+back+1);
  //find the nearest point  计算这个时间的路径长度  找到最近的点并根据插值获得当前的状态坐标
...
bool KinoAstar::IfCollision_Toward(const Eigen::Vector2d &pos, const double &yaw){
  bool occ = false;
  double traj_toword_occ_scope_ = 0.8;
  Eigen::Vector2d point;
  uint8_t check_occ;
  double cos_yaw = cos(yaw);
  double sin_yaw = sin(yaw);
