...
class mpc
{
  ...
  public:
    ...
    int calc_target_index(const Eigen::VectorXd &state_now, const Eigen::VectorXd* refstate, const int &number)
    {
      ...
      for(i=0;i<number;i++){
        dist = pow(refstate[i][0] - state_now[0], 2) + pow(refstate[i][1] - state_now[1], 2) + pow(refstate[i][2]- state_now[2], 2);
        if(dist<dist_min){
          dist_min = dist;
          idx = i;
    ...
    Eigen::VectorXd OSQPsolver(const Eigen::MatrixXd &H, Eigen::VectorXd &g, const Eigen::MatrixXd &A_cons, Eigen::VectorXd &lb, Eigen::VectorXd &ub)
    {
      ...
      Eigen::SparseMatrix<double> hessian(Nu*Nc+1, Nu*Nc+1);
      Eigen::SparseMatrix<double> linearMatrix(Nu*Nc+1, Nu*Nc+1);
    ...
    Eigen::VectorXd getresult(const Eigen::VectorXd &state_now, int &idx, const Eigen::VectorXd* refstate, const Eigen::VectorXd* refinput, const int &number)
    {
      ...
      if(idx!=number-1){
        ...
        // lb <= A*X <= ub
        Eigen::MatrixXd A_cons(Nu*Nc+1, Nu*Nc+1);
        A_cons << A_I, Eigen::MatrixXd::Zero(Nu*Nc,1), Eigen::MatrixXd::Zero(1,Nu*Nc+1);
        A_cons = A_cons + Eigen::MatrixXd::Identity(Nc*Nu+1,Nc*Nu+1);
        // std::cout << "A_cons:" << std::endl << A_cons << std::endl;

        Eigen::VectorXd lb(Nu*Nc+1),lb0(Nu*Nc+1);
        lb << Umin-Ut, 0;
        lb0 << delta_Umin, 0;
        lb = lb + lb0;
        // std::cout << "lb:" << std::endl << lb << std::endl;

        Eigen::VectorXd ub(Nu*Nc+1),ub0(Nu*Nc+1);
        ub << Umax-Ut, 0;
        ub0 << delta_Umax, 1;
        ub = ub + ub0;
